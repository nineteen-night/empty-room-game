...
func AppRun(api server.GameServiceAPI, gameSessionsConsumer, gameStatesConsumer *gameconsumer.GameConsumer) {
    go gameSessionsConsumer.Consume(context.Background())
    go gameStatesConsumer.Consume(context.Background())
...
func runGRPCServer(api server.GameServiceAPI) error {
    ...
    slog.Info("Game Service gRPC server listening on :50052")
    return s.Serve(lis)
}
...
func runGatewayServer() error {
    ...
    slog.Info("Game Service HTTP Gateway server listening on :8081")
