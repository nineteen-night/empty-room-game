...
func InitAuthUpsertConsumer(cfg *config.Config, authProcessor *authprocessor.AuthProcessor) *authconsumer.AuthConsumer {
    kafkaBrokers := []string{fmt.Sprintf("%v:%v", cfg.Kafka.Host, cfg.Kafka.Port)}
    return authconsumer.NewAuthConsumer(authProcessor, kafkaBrokers, cfg.Kafka.UsersUpsertTopic)
}